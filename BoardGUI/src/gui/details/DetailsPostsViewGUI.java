...
import gui.board.posts.PostEditGUI;
import gui.details.DetailsGUI;
import session.UserSession;

public class DetailsPostsViewGUI extends JFrame implements ActionListener {
	...
	// 생성자
	public DetailsPostsViewGUI(TableBoardsDTO board, int postId) {
				...
				if (!c.isDeleted() && (c.getUserId() == currentUserId || isAdmin)) {
					...
					btndel.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							int choice = JOptionPane.showConfirmDialog(DetailsPostsViewGUI.this, "댓글을 삭제하시겠습니까?", "삭제 확인", JOptionPane.YES_NO_OPTION);
							if (choice == JOptionPane.YES_OPTION) {
								int result = commentDAO.deleteComment(c.getCommentId());
								if (result > 0) {
									loadComments();
								} else {
									JOptionPane.showMessageDialog(DetailsPostsViewGUI.this, "댓글 삭제 실패");
								}
							}
						}
					});
